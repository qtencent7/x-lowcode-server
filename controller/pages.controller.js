...
module.exports = {
  ...
  async copy(ctx) {
    ...
    const { name, remark, pageData } = pageInfo;
    await pageService.createPage(name + '-副本', userId, userName, remark, pageData);
    util.success(ctx);
  },
  ...
  async create(ctx) {
    const { userId, userName } = util.decodeToken(ctx);
    const { name, remark, projectId } = ctx.request.body;
    if (!name) {
      return ctx.throw(400, '页面名称不能为空');
    }

    await pageService.createPage(name, userId, userName, remark, '', projectId);
    util.success(ctx);
  },

  async update(ctx) {
    const { id, name, remark = '', pageData, previewImg = '', projectId } = ctx.request.body;
    if (!util.isNotEmpty(id)) {
      return ctx.throw(400, '页面ID不能为空');
    }
    if (!name) {
    ...
    await pageService.updatePageInfo(id, name, remark, pageData, previewImg, projectId);
    util.success(ctx);
  },
